...
class LocalAdbServerDeviceBroker implements DeviceBroker {
  ...
  public BrokeredDevice leaseDevice() {
    ...
    BrokeredDevice device =
        new BrokeredDevice.Builder()
            .withAdbPath(adbPath)
            .withAdbServerPort(adbServerPort.or(DEFAULT_ADB_SERVER_PORT))
  ...
  private void setDextoptFlags(AdbController adbController, String flags) {
    try {
      ...
      logger.log(
          Level.WARNING,
          "Failed to set dalvik.vm.dexopt-flags. Continuing because non-rooted local devices may "
  ...
  private String getSerialNumberFromAdbDevices() {
    ...
    environment.put(
        "ANDROID_ADB_SERVER_PORT", String.valueOf(adbServerPort.or(DEFAULT_ADB_SERVER_PORT)));
  ...
  public Map<String, Object> getExportedProperties() {
    ...
    properties.put("adb_server_port", adbServerPort.or(DEFAULT_ADB_SERVER_PORT));
